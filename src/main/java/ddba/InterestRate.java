...
public class InterestRate {
	...
	/**
	 * it checks what was the interes rate in that date
	 *
	...
	public static double decideInterestPercentage(LocalDate date) {
		if (date.isAfter(listOFInterests.get(listOFInterests.size() - 1).getDate()))
	...
	public static class InterestCalculator {
		static long daysOverDeadline(LocalDate deadlineDate, LocalDate paymentDate) {
		...
		private static Output setupOutput(Invoice invoice, Payment payment, double invoiceTemp) {
			...
			output.setDaysOverDeadline(daysOverDeadline(invoice.getDeadlineDate(), payment.getPaymentDate()));
